...
def compile_project():
  ...
  if config_res.returncode != 0:
    pytest.fail(f"CMake configuration failed!\nSTDOUT: {config_res.stdout}\nSTDERR: {config_res.stderr}")
  ...
  if build_gen_res.returncode != 0:
    pytest.fail(f"Building matrix_generator failed!\nSTDOUT: {build_gen_res.stdout}\nSTDERR: {build_gen_res.stderr}")

  if not os.path.exists(MATRIX_GEN_EXE):
    pytest.fail(f"Critical Error: {MATRIX_GEN_EXE} was not generated by build system!")
